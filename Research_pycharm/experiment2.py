...
def local_model(class1, class2 ):
    ...
    class1label = tf.concat(1, [class1label_1, class1label_2])
    ...
    class2label = tf.concat(1, [class2label_1, class2label_2])


    W_temp = tf.Variable(tf.random_normal([784, 2]), name='W')
    b_temp = tf.Variable(tf.random_normal([2]), name='b')

    y = tf.nn.softmax(tf.matmul(tf.concat(0,[class1input,class2input]), W_temp) + b_temp, name='y')
    sess.run(tf.variables_initializer([class1input,class2input, W_temp, b_temp]))
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(y, tf.concat(0,[class1label,class2label])))
    ...
    class1testlabel = tf.concat(1, [class1testlabel_1, class1testlabel_2])
    ...
    class2testlabel = tf.concat(1, [class2testlabel_1, class2testlabel_2])
    # compare predicted label and actual label
    test_predicted=tf.nn.softmax(tf.matmul(tf.concat(0,[class1test,class2test]), W_temp) + b_temp, name='test_predicted')

    correct_prediction = tf.equal(tf.argmax(test_predicted, 1), tf.argmax(tf.concat(0,[class1testlabel,class2testlabel]), 1))
    ...
    y_actual=tf.concat(0, [tf.constant(numpy.array(train_labels_originals_class1)), tf.constant(numpy.array(train_labels_originals_class2))])
    ...
    return W_temp,b_temp, tf.concat(0,[class1input,class2input]), y, y_actual
...
W2,b2, traininput2,trainpredictions2,trainlabels2=local_model(2,6)
W3,b3, traininput3,trainpredictions3,trainlabels3=local_model(3,4)
W4,b4, traininput4,trainpredictions4,trainlabels4=local_model(9,5)
W5,b5, traininput5,trainpredictions5,trainlabels5=local_model(8,7)
...
level2traininput=tf.concat(1,[traininputlevel1,traininputpredictions1,traininputpredictions2,traininputpredictions3,traininputpredictions4,traininputpredictions5])
print(level2traininput)

level2W = tf.Variable(tf.zeros([794, 10]),name='W')
...
level2cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(level2y, level2y_))
...
level2testinput=tf.concat(1,[testinputlevel1,testinputpredictions1,testinputpredictions2,testinputpredictions3,testinputpredictions4,testinputpredictions5])
